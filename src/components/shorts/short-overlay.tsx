...
export function ShortOverlay({
  ...
  muted: boolean;
  onToggleMute: () => void;
  /** Enriched from the shorts card (no detail request) — rendered as caption links. */
  actors?: { name: string; slug: string }[];
  ...
  return (
    <>
      ...
      <div className="absolute right-0 bottom-0 left-0 z-10 flex items-end justify-between gap-3 bg-gradient-to-t from-black/70 to-transparent p-4 pb-6">
        <div className="flex max-w-[80%] flex-col gap-1.5" onClick={(e) => e.stopPropagation()}>
          <h2 className="line-clamp-2 text-sm font-semibold text-white drop-shadow">{title}</h2>
          {actors.length > 0 ? (
            <div className="flex flex-wrap gap-x-2 text-xs font-medium text-white/90 drop-shadow">
              {actors.slice(0, 2).map((a) => (
                <Link key={a.slug} href={`/actor/${a.slug}`} className="hover:underline">
                  @{a.name}
                </Link>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {tags.slice(0, 3).map((tag) => (
                <Link
                  key={tag.slug}
                  href={`/tag/${tag.slug}`}
                  className="rounded-full bg-white/20 px-2 py-0.5 text-[11px] font-medium text-white backdrop-blur hover:bg-white/30"
                >
                  #{tag.name}
                </Link>
              ))}
